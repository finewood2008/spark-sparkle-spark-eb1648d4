          ...
          {contents.map(item => (
            ...
            >
              <div className="font-medium truncate flex items-center gap-1">
                {item.autoGenerated && <Bot size={12} className="text-spark-orange shrink-0" />}
                {item.title || '未命名草稿'}
              </div>
              <div className="text-[10px] text-spark-gray-400 mt-0.5 flex items-center gap-1">
                {item.platform === 'xiaohongshu' ? '小红书' : item.platform === 'wechat' ? '公众号' : '抖音'}
                {' · '}
                {item.status === 'draft' ? '草稿' : item.status === 'published' ? '已发布' : item.status === 'approved' ? '已审核' : '审核中'}
                {item.autoGenerated && <span className="bg-spark-orange/10 text-spark-orange px-1 rounded">AI</span>}
              </div>
            </button>
          ))}
        </div>
          ...
          <>
            <div className="p-4 border-b border-spark-gray-200 flex items-center gap-3">
              {selectedContent.autoGenerated && (
                <span className="inline-flex items-center gap-1 bg-spark-orange/10 text-spark-orange text-xs font-medium px-2 py-0.5 rounded-full">
                  <Bot size={12} /> AI 生成
                </span>
              )}
              <select
              ...
              {selectedContent.status === 'published' ? (
                ...
                <button
                  ...
                  className="spark-btn-primary text-xs flex items-center gap-1"
                >
                  <Send size={14} /> 发布
                </button>
              )}
